...
function hookLoader(matcherOrRoot, opts) {
    ...
    dirPath = matcherOrRoot;
    setInterval(function(){
        var path = matcherOrRoot + '/coverage/coverage-' + fileSuffix + '.json';
        var coverage = getCoverageObject();
        fs.writeFile(path, JSON.stringify(coverage, null, 2), 'utf8');
    }, 3000);
}
...
function render(filePath, res, prefix) {
    ...
    // get all coverage
    fs.readdir(dirPath + '/coverage', function(err, files) {
        if(!!err || files.length  === 0) {
            res.setHeader('Content-type', 'text/plain');
            return res.end('No coverage information has been collector');
        }

        files.forEach(function(file) {
	    var path = dirPath + '/coverage/' + file;
	    delete require.cache[path];
            collector.add(require(path));
        });

        // convenience method: do not use this when dealing with a large number of files
        var finalCoverage = collector.getFinalCoverage();

        utils.removeDerivedInfo(finalCoverage);
        ...
        if (outputNode.kind === 'dir') {
            ...
            fileCoverage = finalCoverage[outputNode.fullPath()];
